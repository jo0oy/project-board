...
public class ArticleCommandServiceImpl implements ArticleCommandService {
    ...
    @Override
    @Transactional
    public ArticleInfo.MainInfo registerArticle(String username, ArticleCommand.RegisterReq command) {
        ...
        return new ArticleInfo.MainInfo(articleRepository.save(command.toEntity(userAccount)));
    }
    ...
    @Override
    @Transactional
    public void update(Long articleId, String username, ArticleCommand.UpdateReq command) {
        log.info("{}:{}", getClass().getSimpleName(), "update(Long, String, ArticleCommand.UpdateReq)");

        // update 할 Article 엔티티 조회
        var article = articleRepository.findByIdFetchJoinUserAccount(articleId)
                .orElseThrow(() -> new EntityNotFoundException("존재하지 않는 게시글입니다."));

        // update 권한 확인하기 위한 UserAccount 엔티티 조회
    ...
    public void delete(Long articleId, String username) {
        ...
        // delete 권한 확인하기 위한 UserAccount 엔티티 조회
        var userAccount = userAccountRepository.findByUsername(username)
                .orElseThrow(() -> new UsernameNotFoundException("유저가 존재하지 않습니다. username=" + username));
